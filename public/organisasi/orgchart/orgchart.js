...
(function ($) {
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append('<div><img class="cover" src="https://cdn2.iconfinder.com/data/icons/diagona/icon/16/130.png"/></div>');
		}
		...
		if ($childNodesCount > 0) {
			$nodeDiv.next().children('img.cover').click(function () {
				...
				if ($tr.hasClass('contracted')) {
					...
					$(this).attr('src', 'images/https://cdn2.iconfinder.com/data/icons/diagona/icon/16/130.png');
					...
					$(this).attr('src', 'http://icons.iconarchive.com/icons/martz90/circle-addon1/16/text-plus-icon.png');
		...
		if ($node.attr('class') != undefined) {
			...
			$.each(classList, function (index, item) {
				if (item == 'collapsed') {
					...
					$nodeRow.find('img.cover').attr('src', 'images/http://icons.iconarchive.com/icons/martz90/circle-addon1/16/text-plus-icon.png');
				} else {
