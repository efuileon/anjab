...
<section class="content-header">
         <h1>
           Daftar Pengembalian Kenaikan Pangkat
         </h1>
</section>
<br>
                ...
                <table id="example1" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                  ...
                                  <td>
                                    ...
                                    <a href="{{asset($berkas_cpns->lokasi)}}">{{$berkas_cpns->filename}}</a>->{{$berkas_cpns->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_pns as $berkas_pns)
                                    <a href="{{asset($berkas_pns->lokasi)}}">{{$berkas_pns->filename}}</a>->{{$berkas_pns->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_pangkat as $berkas_pangkat)
                                    <a href="{{asset($berkas_pangkat->lokasi)}}">{{$berkas_pangkat->filename}}</a>->{{$berkas_pangkat->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_ijazah as $berkas_ijazah)
                                    <a href="{{asset($berkas_ijazah->lokasi)}}">{{$berkas_ijazah->filename}}</a>->{{$berkas_ijazah->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_transkrip as $berkas_transkrip)
                                    <a href="{{asset($berkas_transkrip->lokasi)}}">{{$berkas_transkrip->filename}}</a>->{{$berkas_transkrip->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_pak as $berkas_pak)
                                    <a href="{{asset($berkas_pak->lokasi)}}">{{$berkas_pak->filename}}</a>->{{$berkas_pak->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_skp1 as $berkas_skp1)
                                    <a href="{{asset($berkas_skp1->lokasi)}}">{{$berkas_skp1->filename}}</a>->{{$berkas_skp1->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_skp2 as $berkas_skp2)
                                    <a href="{{asset($berkas_skp2->lokasi)}}">{{$berkas_skp2->filename}}</a>->{{$berkas_skp2->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_jabfung as $berkas_jabfung)
                                    <a href="{{asset($berkas_jabfung->lokasi)}}">{{$berkas_jabfung->filename}}</a>->{{$berkas_jabfung->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_jabstruk as $berkas_jabstruk)
                                    <a href="{{asset($berkas_jabstruk->lokasi)}}">{{$berkas_jabstruk->filename}}</a>->{{$berkas_jabstruk->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_drj as $berkas_drj)
                                    <a href="{{asset($berkas_drj->lokasi)}}">{{$berkas_drj->filename}}</a>->{{$berkas_drj->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_uraian as $berkas_uraian)
                                    <a href="{{asset($berkas_uraian->lokasi)}}">{{$berkas_uraian->filename}}</a>->{{$berkas_uraian->catat_ver}}<br>
                                    @endforeach
                                    @foreach($berkas_lain as $berkas_lain)
                                    <a href="{{asset($berkas_lain->lokasi)}}">{{$berkas_lain->filename}}</a>->{{$berkas_lain->catat_ver}}<br>
                                    @endforeach
                                  </td>
                                  <td>* {{$ukp->catatan}}<br>* {{$ukp->ver_catatan}}</td>
                                  <td><a href ="{{url('pangkat/uploadreg')."/".$ukp->id}}" class='btn btn-success'></i><i class="fa fa-edit"></i> Cek</a></td>
                                  <td><a href ="{{url('pangkat/kirim_revisi')."/".$ukp->id}}" class='btn btn-primary'></i><i class="fa fa-edit"></i> Kirim BKPSDM</a></td>
                                  @php $i++ @endphp
                                  @endforeach
                                </tbody>
                            </table>
